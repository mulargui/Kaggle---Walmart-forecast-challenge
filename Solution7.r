...
#use a seasonal neural network model
...
for (i in 1:45)
{
	...
	for (j in 1:99)
	{
		...
		# no data
		if(nrow(dfsample) == 0) next
		#not enough data, use last year sales as a predictor
		if((nrow(dfsample) < 3) | ((i==21)&(j==78)))
		{
			dfresult <- merge(dftest, dfsample, by.x = c("Store","Dept", "Seasonality", "PYear"), 
				by.y = c("Store","Dept", "Seasonality", "Year"))
			dfpredict <- rbind(dfpredict, subset(dfresult, select = c(Store, Dept, Week.x, Weekly_Sales)))
			next
		}

		fit <- nnetar(dfsample$Weekly_Sales)
...
write.csv(file = "Solution7.csv", x = subset(dfresult, select = c(Id, Weekly_Sales)), row.names=FALSE)