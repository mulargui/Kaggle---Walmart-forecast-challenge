...
#adjust sales in weeks 46, 47, 48 and 49 in 2011 to have same number of sale days in week 49 than in 2012
dfchristmas <- dftrain %>% filter(Year == 2011) %>% filter((Seasonality == 46) | (Seasonality == 47) | (Seasonality == 48) | (Seasonality == 49))
...
for (i in 1:45)
{
	...
	for (j in 1:99)
	{
			...
			if(nrow(dfsample) < 4) 
			{
				inc_sales <- sum(dfsample$Weekly_Sales) / 21	
				old <- dfsample$Weekly_Sales[(dfsample$Seasonality == 47)]
				dftrain$Weekly_Sales[(dftrain$Store==i)&(dftrain$Dept == j)&(dftrain$Seasonality == 47)&(dftrain$Year == 2011)] <- old - inc_sales
				...
				dftrain$Weekly_Sales[(dftrain$Store==i)&(dftrain$Dept == j)&(dftrain$Seasonality == 49)&(dftrain$Year == 2011)] <- old + (2 * inc_sales)
				next
			}
			inc_sales <- sum(dfsample$Weekly_Sales) / 28	
			old <- dfsample$Weekly_Sales[(dfsample$Seasonality == 46)]
			dftrain$Weekly_Sales[(dftrain$Store==i)&(dftrain$Dept == j)&(dftrain$Seasonality == 47)&(dftrain$Year == 2011)] <- old - inc_sales
			old <- dfsample$Weekly_Sales[(dfsample$Seasonality == 47)]
			dftrain$Weekly_Sales[(dftrain$Store==i)&(dftrain$Dept == j)&(dftrain$Seasonality == 47)&(dftrain$Year == 2011)] <- old - inc_sales
			#old <- dfsample$Weekly_Sales[(dfsample$Seasonality == 48)]
...
write.csv(file = "Solution10.csv", x = subset(dfresult, select = c(Id, Weekly_Sales)), row.names=FALSE)